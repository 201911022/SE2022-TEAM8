...
public class UserService implements UserDAO{
	...
	public boolean updatePassword(HashMap<String, String> map, String id) {
	...
	public boolean updateInfo(HashMap<String, String> map, String id) {
		Optional<UserEntity> user = this.userRepository.findByUserId(id);
		UserEntity newUser = user.get();
		String encryptedPassword = encoder.encode(map.get("pw"));
		if (user.isPresent() && encoder.matches(map.get("pw"), newUser.getPw())) {
			newUser.setPw(encryptedPassword);
